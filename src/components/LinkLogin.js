...
const LinkLogin = () => {
  ...
  const fetchAvailability = async (requestIdFromQuery) => {
    try {
      const response = await fetch(`/api/open-or-taken?requestId=${requestIdFromQuery}`);
      if (response.ok) {
        ...
        setDay(data.day || '');
        setNotes(data.notes || '');
        setBlockDetails(data.block_details || []);
        setAvailable(data.available || false);
        setError('');
        toast({
          title: data.available ? 'Request Available' : 'Request Fully Taken',
          description: data.available
            ? 'Some blocks are available to accept.'
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const assignResponse = await fetch('/api/assign-substitute', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ token, password, requestId, blocks: selectedBlocks }),
      });
      ...
      if (assignResponse.status !== 200 || !assignData.added) {
        ...
        if (assignResponse.status === 400) {
          ...
          errorMsg = assignData.error || 'Some blocks already taken';
      ...
      toast({
        title: 'Success',
        description: 'Blocks accepted and logged in successfully!',
        status: 'success',
        duration: 5000,
        isClosable: true,
        bg: inputBg,
      ...
      {hasAccepted ? (
        <Card bg={inputBg} boxShadow="lg" borderRadius="lg" maxW="600px" w="full" mb={6}>
          <CardBody textAlign="center">
            ...
            <Text mb={4} color={mainColor}>You have successfully accepted the selected blocks.</Text>
            <Button
              bg={accentColor}
              color={mainColor}
        ...
        <Card bg={inputBg} boxShadow="lg" borderRadius="lg" maxW="400px" w="full" mb={6}>
          <CardBody>
            ...
            <VStack as="form" spacing={4} onSubmit={handleSubmit}>
              ...
              {available && (
                <FormControl>
                  <FormLabel color={mainColor}>Select Available Blocks</FormLabel>
                  <CheckboxGroup onChange={setSelectedBlocks} value={selectedBlocks}>
                    <VStack align="start">
                      {blockDetails.map((b, i) => (
                        <Checkbox
                          key={i}
                          value={b.name}
                          isDisabled={!b.available}
                          colorScheme="green"
                        >
                          {b.name} {!b.available && `(Taken by ${b.assigned_to})`}
                        </Checkbox>
                      ))}
                    </VStack>
                  </CheckboxGroup>
              ...
              <Button
                ...
                size="lg"
                w="full"
                isDisabled={!available || isLoading || selectedBlocks.length === 0}
                isLoading={isLoading}
              >
                Accept Selected Blocks
              </Button>
              {error && (
      ...
      <Card bg={inputBg} boxShadow="lg" borderRadius="lg" maxW="600px" w="full">
        <CardBody>
          ...
          <Grid templateColumns={{ base: '1fr', sm: '1fr 1fr' }} gap={4}>
            ...
            <GridItem>
              <Text fontWeight="bold" color={mainColor}>Subject:</Text>
            ...
            <GridItem colSpan={{ base: 1, sm: 2 }}>
              ...
              <Text color={mainColor}>{notes || 'None'}</Text>
            </GridItem>
            <GridItem colSpan={{ base: 1, sm: 2 }}>
              <Text fontWeight="bold" color={mainColor}>Blocks Requested:</Text>
              {blockDetails.map((b, i) => (
                <Text key={i} color={b.available ? 'green.500' : 'red.500'}>
                  {b.name} {!b.available && `- Taken by ${b.assigned_to}`}
                </Text>
              ))}
            </GridItem>
