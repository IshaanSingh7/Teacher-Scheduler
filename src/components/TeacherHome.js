  ...
  useEffect(() => {
    ...
    const fetchData = async () => {
      try {
        ...
        if (response.ok) {
          const data = await response.json();
          // Each request now includes a `sent` field (string from Users.sent)
          console.log("TEACHER REQUESTS:", data);        // ← ADD THIS
          console.log("Sent fields:", data.map(r => r.sent));  // ← ADD THIS
          setRequests(data);
        } else {
          toast({
            title: 'Error',
                ...
                <Tbody>
                  {requests.length > 0 ? (
                    requests.map((request) => {
                      ...
                      const [year, month, day] = (request.day || '').split('-');
                      const formattedDate = year && month && day ? `${month}-${day}-${year}` : (request.day || '-');
                      return (
                        <Tr key={request.id}>
                          <Td color={mainColor} whiteSpace="nowrap">
                            {formattedDate}
                    ...
                    <VStack align="start" spacing={4} w="full">
                      ...
                      <Box w="full">
                        ...
                        {assignedBlocks.length > 0 ? (
                          ...
                          <Text fontSize="sm" color="gray.500">No substitutes assigned yet.</Text>
                        )}
                      </Box>

                      {/* Notification recipients from Users.sent */}
                      <Box w="full" pt={2} borderTop="1px" borderColor="gray.300">
                        <Text fontWeight="bold" mb={1}>Notification Recipients</Text>
                        {(subRequest.sent && subRequest.sent.length > 0) ? (
                          <VStack align="start" spacing={1}>
                            {subRequest.sent.map((email, i) => (
                          ...
                          <Text fontSize="sm" color="gray.500">No recipients recorded.</Text>
                        )}
                      </Box>
                    </VStack>
                  );
