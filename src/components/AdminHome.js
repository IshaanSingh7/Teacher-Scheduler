...
const AdminHome = () => {
  ...
  const handleAction = async (requestId, type) => {
    try {
      ...
      if (type === 'complete') {
        // Complete request uses POST with body
        ...
        // Cancel request uses DELETE with query params, no body, no headers
        url = `/api/requests/${requestId}?email=${encodeURIComponent(user.email)}`;
      ...
      if (data.added || data.deleted || data.message) {
          ...
          <AlertDialogContent bg={inputBg}>
            ...
            <AlertDialogBody color={mainColor}>
              {subRequest && subRequest.blocks ? (() => {
                const assignedBlocks = subRequest.blocks.filter(b => b.assigned).sort((a, b) => getBlockNumber(a.block) - getBlockNumber(b.block));
                return assignedBlocks.length > 0 ? (
                  <VStack align="start" spacing={2}>
                    {assignedBlocks.map(block => (
                      <Text key={block.block}>{block.block}: {block.substitute_name} (<a href={`mailto:${block.substitute_email}`}>{block.substitute_email || 'Email not available'}</a>)</Text>
                    ))}
                  </VStack>
